defmodule SiplaneWeb.API.Runner.V0.JobController do
  ...
  def update_state(conn, params) do
    ...
    with {:ok, validated_job_id_params} <- Tarams.cast(params, @job_id_params_schema),
	 {:ok, validated_body} <- Tarams.cast(conn.body_params, @update_state_body_schema) do
      ...
      Siplane.Job.update_job_state(validated_job_id_params.id, validated_body)
      send_resp(conn, 204, "")
    else
      {:error, errors} ->
  ...
  def put_console_log(conn, params) do
    with {:ok, validated_job_id_params} <- Tarams.cast(params, @job_id_params_schema),
         [offset_hdr_val] <- Plug.Conn.get_req_header(conn, "x-siplane-console-offset"),
         {offset, ""} <- Integer.parse(offset_hdr_val, 10),
         [next_hdr_val] <- Plug.Conn.get_req_header(conn, "x-siplane-console-next"),
         {next, ""} <- Integer.parse(next_hdr_val, 10),
	 {:ok, body, conn} = Plug.Conn.read_body(conn) do
      Siplane.Job.put_console_log(validated_job_id_params.id, offset, next, body)
      send_resp(conn, 204, "")
    else
      {:error, errors} ->
	conn
	...
	|> put_view(SiplaneWeb.API.ErrorView)
	|> render("bad_request.json",
	  description: "Missing or invalid X-Siplane-Console-Offset or X-Siplane-Console-Next headers."
	)
      :error ->
	conn
	|> put_status(:bad_request)
