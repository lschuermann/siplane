defmodule Treadmill.Board.Server.JobState do
  defstruct [
...
defmodule Treadmill.Board.Server do
  use GenServer
  import Ecto.Query
  ...
  # API is provided by the Treadmill.Board module (which manages these servers,
  ...
  defp complete_jobs(state, job_ids, completion_code) do
    ...
    Treadmill.Repo.update_all(
      from(j in Treadmill.Job, where: j.id in ^ecto_job_ids),
    ...
    for job_id <- job_ids do
      :ok = Registry.unregister(Treadmill.Job.BoardRegistry, job_id)
    end
  ...
  defp notify_board_update(state, what) do
    Registry.dispatch(Treadmill.Board.SubscriberRegistry, state.board_id, fn subscribers ->
  ...
  defp schedule_jobs(state) do
    if map_size(state.active_jobs) < state.max_jobs do
      ...
      case Treadmill.Job.pending_jobs(board_id: state.board_id, limit: 1) do
	[] -> state
	[job] ->
  ...
  def init(board_id) do
    ...
    # for it.
    case Treadmill.Repo.exists?(from b in Treadmill.Board, where: b.id == ^Ecto.UUID.load!(board_id)) do
      false ->
	...
	case Registry.register(__MODULE__.Registry, board_id, nil) do
	  ...
	  {:ok, _} ->
	    ...
	    from(j in Treadmill.Job, where: j.board_id == ^Ecto.UUID.load!(board_id) and j.dispatched == true and is_nil(j.completion_code))
	    |> Treadmill.Repo.update_all(set: [completion_code: :server_crashed, updated_at: DateTime.utc_now()])
  ...
  def handle_call({:connect_runner, conn_info}, {runner_pid, _}, state) do
    ...
    Treadmill.Log.info(
      %Treadmill.Log{
	severity: Treadmill.Log.severity_info,
