defmodule Treadmill.Job do
  ...
  def active_jobs(opts) do
	...
	board_id ->
	  ...
	  # if we have one.
	  case Registry.lookup(Treadmill.Board.Server.Registry, board_id) do
	    [{pid, _val}] ->
    ...
    if Keyword.get(opts, :with_dispatched_crashed) == true do
      Treadmill.Repo.all(from j in Treadmill.Job, where: (j.id in ^ecto_job_ids) or (is_nil(j.completion_code) and j.dispatched == true))
    else
      Treadmill.Repo.all(from j in Treadmill.Job, where: j.id in ^ecto_job_ids)
    end
  end
  ...
  def instant_job(board_id, environment_id, opts) do
    ...
    Treadmill.Board.jobs_updated(board_id)
  ...
  def job_log(job_id, limit \\ 50) do
    ...
    query = from jl in Treadmill.Job.LogEvent,
